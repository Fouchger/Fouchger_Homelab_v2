# ================================================================
# File: packer/proxmox/debian/debian-12.pkr.hcl
# Purpose:
#   Build the Debian 12 Proxmox template using the curated netinst ISO
...
source "proxmox-iso" "debian-12" {
  ...
  vm_name                  = local.template_names.debian_12
  template_name            = local.template_names.debian_12
  template_description     = local.template_descriptions.debian_12
  task_timeout             = var.task_timeout
  tags                     = join(";", concat(var.common_tags, ["debian", "debian-12", "linux"]))
  ...
  boot_iso {
    type         = "scsi"
    iso_file     = "${var.proxmox_iso_storage}:iso/${var.debian_12_iso_filename}"
    iso_checksum = var.debian_12_iso_checksum
    unmount      = true
  }

  http_content = {
    "/debian-12/preseed.cfg" = templatefile("${path.root}/debian/http/debian-preseed.pkrtpl.hcl", {
      hostname            = local.template_names.debian_12
